...
In the last chapter, we learnt how to use a set of coordinates to represent a rigid body's position in three-dimensional space. However, that's not enough to describe the rigid-body's motion in the space. In this chapter, a more systematic method will be introduced to "describe a rigid body's position and orientation which relies on attaching a reference frame to the body" and using a $4\times 4$ matrix to represent the configuration of reference frame with respect the fixed frame, which is an implicit representation as mentioned in the last chapter.
